...
- `relay_management_client.zig`
  - goal: compose `NIP-86` `supportedmethods`, `listallowedpubkeys`, `allowpubkey`,
    `listallowedkinds`, `allowkind`, and `banpubkey` requests over the explicit HTTP post seam with
    caller-driven `NIP-98` authorization setup, including one SDK-prepared convenience path and
