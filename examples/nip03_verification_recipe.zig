...
// classify remembered discovery entries for freshness, inspect the remembered runtime action, then
// recover the latest remembered verification for the same target event.
test "recipe: sdk opentimestamps verifier fetches, remembers, classifies freshness, inspects remembered runtime, and reuses one detached proof document" {
