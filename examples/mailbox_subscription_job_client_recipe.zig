...
// Prepare one mailbox live-subscription job that either authenticates the relay or starts one
// bounded live mailbox transcript turn, then close it explicitly.
test "recipe: mailbox subscription job client authenticates then subscribes explicitly" {
    ...
    var client_storage = noztr_sdk.client.MailboxSubscriptionJobClientStorage{};
    var client = noztr_sdk.client.MailboxSubscriptionJobClient.init(.{
    ...
    const outbound = try sender_session.beginDirectMessage(
        ...
        &.{
            ...
            .content = "mailbox subscription job recipe payload",
            .created_at = 41,
    ...
    var auth_storage = noztr_sdk.client.MailboxSubscriptionJobAuthEventStorage{};
    ...
    try std.testing.expect(intake.envelope != null);
    try std.testing.expectEqualStrings(
        "mailbox subscription job recipe payload",
        intake.envelope.?.direct_message.message.content,
    );
}
