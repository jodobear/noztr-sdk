...
// Replay one mailbox wrap transcript explicitly, accept the parsed wrap event through mailbox
// intake, then close the replay turn with one explicit checkpoint candidate.
test "recipe: mailbox replay turn client accepts wrapped replay events and returns checkpoint closure" {
