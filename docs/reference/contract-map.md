...
| Explicit `NIP-86` relay-management request/result handling over the HTTP post seam with caller-owned auth preconditions, including both caller-built and SDK-prepared `NIP-98` authorization headers | `noztr_sdk.client.relay.management.Client` | [relay_management_client.zig](../../examples/relay_management_client.zig) |
