...
| Explicit `NIP-86` relay-management request/result handling over the HTTP post seam with caller-owned auth preconditions, including relay-name/description/icon metadata mutation, pubkey allow/ban mutation, kind allow mutation, event/IP moderation mutation, typed moderation-needed plus banned-event/pubkey/IP list reads, and both caller-built and SDK-prepared `NIP-98` authorization headers | `noztr_sdk.client.relay.management.Client` | [relay_management_client.zig](../../examples/relay_management_client.zig) |
