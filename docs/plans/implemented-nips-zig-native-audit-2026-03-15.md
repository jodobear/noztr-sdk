...
- `MailboxDeliveryPlan` now also exposes recipient-only and sender-copy-only next-step selectors so
  callers can follow those two delivery roles directly instead of rebuilding that filtering above
  the workflow
...
- `GroupFleet.nextPublishEvent(...)` and `nextPublishStep(...)` now expose one explicit next
  publish target above that fanout so callers can step one publish relay without hand-scanning the
  returned fleet slice or re-stitching event context above the workflow
...
- `IdentityVerifier.inspectStoredProfilePolicyForTargets(...)` now also groups one caller-owned
