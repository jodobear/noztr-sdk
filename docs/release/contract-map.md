...
| Restore one relay-local `NIP-29` group snapshot over the shared event seam | `noztr_sdk.store`, `RelayLocalGroupArchive`, `MemoryClientStore`, `GroupClient` | `noztr_sdk.store.RelayLocalGroupArchive` | [relay_local_group_archive_recipe.zig](/workspace/projects/nzdk/examples/relay_local_group_archive_recipe.zig) |
| Inspect shared multi-relay readiness and select one typed next runtime step | `noztr_sdk.runtime`, `RelayPool`, `RelayPoolStorage`, `RelayPoolPlanStorage`, `RelayPoolPlan`, `RelayPoolStep` | `noztr_sdk.runtime.RelayPool` | [relay_pool_recipe.zig](/workspace/projects/nzdk/examples/relay_pool_recipe.zig) |
| Export and restore shared relay-pool membership plus per-relay cursors over the shared checkpoint seam | `noztr_sdk.runtime`, `RelayPool`, `RelayPoolCheckpointStorage`, `RelayPoolCheckpointSet`, `RelayPoolCheckpointStep`, `noztr_sdk.store.RelayCheckpointArchive` | `noztr_sdk.runtime.RelayPool` | [relay_pool_checkpoint_recipe.zig](/workspace/projects/nzdk/examples/relay_pool_checkpoint_recipe.zig) |
| Remote signing, request/response orchestration, relay switching, and shared relay-pool inspection | `RemoteSignerSession`, `RemoteSignerRequestContext`, `RemoteSignerPubkeyTextRequest`, `RemoteSignerTextResponse`, `RemoteSignerRelayPoolRuntimeStorage`, `noztr_sdk.runtime.RelayPoolStep` | `noztr_sdk.workflows.RemoteSignerSession` | [remote_signer_recipe.zig](/workspace/projects/nzdk/examples/remote_signer_recipe.zig) |
| Private-message send, receive, delivery planning, mailbox workflow stepping, and shared relay-pool inspection | `MailboxSession`, `MailboxDeliveryPlan`, `MailboxRuntimePlan`, `MailboxWorkflowPlan`, `MailboxRelayPoolRuntimeStorage`, `MailboxFileMessageRequest`, `noztr_sdk.runtime.RelayPoolStep` | `noztr_sdk.workflows.MailboxSession` | [mailbox_recipe.zig](/workspace/projects/nzdk/examples/mailbox_recipe.zig) |
