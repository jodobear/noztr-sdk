...
- [relay_management_client.zig](../examples/relay_management_client.zig)
  - first explicit `NIP-86` admin request/result route over the shared HTTP seam, including typed
    allowed-pubkey, moderation-needed, banned-event, and blocked-IP inspection plus explicit
    pubkey, event, and IP moderation mutation, using the SDK-prepared auth path while keeping
    lower-level prepared-header auth available on the client surface
